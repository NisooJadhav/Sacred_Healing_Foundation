...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden border" style={{ background: 'var(--color-lotus-section)', boxShadow: 'var(--shadow-sacred)' }}>
      <div className="relative w-full h-44 sm:h-56" style={{ background: 'var(--color-lotus-bg)' }}>
        {product.image ? (
          // Image paths are served from /public
          <Image src={product.image} alt={product.title} fill className="object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center text-purple-300">No image</div>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        {product.category && (
          <div className="inline-block text-xs px-2 py-1 rounded-full text-white mt-2" style={{ background: 'var(--color-lotus-purple)' }}>
            {product.category}
          </div>
        )}
        {product.price && <div className="text-purple-600 font-bold mt-3">{product.price}</div>}
